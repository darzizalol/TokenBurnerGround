...
4. Otherwise, take the TOP unclaimed task in the active project's `BACKLOG.md` (the `ACTIVE:` line of root `PROJECTS.md` names the project directory; task paths are relative to it). Mark it `[claimed <UTC timestamp>]`, commit that one-line edit to main (`engineer: claim <task>`) and push, so no other session grabs it.
...
6. Implement the task **with tests**, following the acceptance criteria exactly. Prefer stdlib; justify any new dependency in one sentence in the PR body. Run the full test suite from the active project's directory inside the worktree (e.g. `<worktree>/projects/<slug>/`). Do not open a PR with failing tests.
