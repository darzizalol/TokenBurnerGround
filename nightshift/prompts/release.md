...
3. Run `gh pr list` and inspect each open PR's comments (`gh pr view <n> --comments`):
   - **Merge** if, since the latest push, there is both `VERDICT: LGTM` and `QA: PASS`: `gh pr merge <n> --squash --delete-branch`. Then mark the task done in `BACKLOG.md`.
   - **Bounce-count**: if a PR has accumulated 3 or more `VERDICT: CHANGES REQUESTED` / `QA: FAIL` verdicts in total, close it with a comment explaining why, and move its task to `## Graveyard` in `BACKLOG.md` with a one-line post-mortem.
