...
## 1. Standard library: `nth_catalan` — the k-th Catalan number by position [claimed 2026-08-25T14:43:25Z]
...
**Ordering note:** guards in `match` arms (`n if n > 0 => ...`) was
attempted and closed after three failed review rounds — see
`## Graveyard` below — so it is no longer ahead of this task in the
queue and won't change `MatchArm`'s field list before this lands.
Nothing else queued ahead of this task touches `MatchArm`, so the
sketch below should still match current `main` when claimed; adapt to
whatever the merged code actually looks like if that's changed by the
time you pick this up, the same way `nth_triangular` (#313) adapted to
...
directly, post-#312), so the *principle* is exact even if
...
## 6. Language: negative literal patterns in `match` arms (`-5 => "neg"`)
