...
## 1. Standard library: `is_achilles` — powerful but not itself a perfect power [claimed 2026-08-19T19:42:13Z]
...
## 2. Language: named function expressions (`fn name(params) { ... }`) for self-referencing anonymous functions
...
## 3. Standard library: `is_pernicious` — a number whose binary popcount is itself prime
...
## 4. Language: inclusive range literal `a..=b` as sugar for `range(a, b + 1)`
...
## 5. Standard library: `is_sphenic` — a number that is the product of three distinct primes
...
## 6. Language: triple-quoted string literals `"""..."""`/`'''...'''`
