...
a `^` (symmetric difference) operator for lists (#373, the set-style
...
`is_disarium`'s own digit-position math); a `|` (union) operator for
lists (#371, the set-style counterpart to list `&`/`-`, mirroring the
existing `union()` builtin's dedupe-and-concatenate semantics).
...
after list `^` #373 landed and dropped it to 5, right at the floor —
one task added to bring it back to the 6-task ceiling, and two stale
cross-references left over from prior renumbering — task 2 citing a
now-merged "task 1", task 4 citing itself as "task 4" — corrected to
name the actual PRs they meant): breadth — `is_self_number`, the
...
sum-of-squares cycle detection; depth — a `|` (union) operator for
maps, the map-side counterpart list `|` leaves deferred, following the
same "left's values win" convention map `&`/`-` already established
rather than `+`'s right-wins merge semantics; breadth —
...
`prime_factors`/`is_smith_number` already use; depth (added this
pass) — a `<=>` (spaceship / three-way comparison) operator, returning
`-1`/`0`/`1` by composing the existing `<` and `==` comparison
machinery, so every already-comparable type (numbers, strings, lists,
maps) gets it for free. Queue now runs breadth, depth, breadth, depth,
breadth, depth. `main` is green (4183 tests), PR queue empty.
