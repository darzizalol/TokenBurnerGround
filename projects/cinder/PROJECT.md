...
numbers-only like `sum` rather than `min_by`/`max_by`'s number-or-string
key), `reject` as `filter`'s predicate-inverted complement (mirroring
how `omit`/`omit_by` already closed that same gap for `pick`/`pick_by`),
