...
- **Control flow**: `if`/`else`, `while` with an optional trailing
  ...
  depth, composable with per-key rename and rest capture), and default
  values for trailing elements in list patterns (`match ([1]) { [a, b =
  ...
  nesting), whole-value `as` binding on list/map patterns
  ...
  scoped per-arm), and the same whole-value `as` binding extended to
  literal and range patterns (`match (5) { 1..10 as whole => whole, _ =>
...
- **Operators**: full arithmetic/comparison/logical set, unary `+`
  ...
  target works for free through the same desugaring), and map
  intersection via the same `&` operator (`{"a": 1, "b": 2} & {"b": 3,
  "c": 4}` is `{"b": 2}`, a fresh non-mutating map keeping only keys
  present in both maps, with the left-hand map's values winning on
  overlap — key-based semantics mirroring map `-`'s own "keys decide,
  left's values win" convention, the map-typed sibling of list `&`
  above; `&=` on a map target works for free through the same
  desugaring; `&` remains int-only bitwise-AND when neither operand is
  a list or a map), and list union via `|` (`[1, 2, 3] | [2, 3, 4]` is
  `[1, 2, 3, 4]`, a fresh non-mutating list combining both sides and
  deduping the result — set-style semantics mirroring the existing
  `union()` builtin, the list-typed counterpart to list `&`/`-` above;
  `|=` on a list target works for free through the same desugaring;
  `|` remains int-only bitwise-OR when neither operand is a list), and
  map union via the same `|` operator (`{"a": 1} | {"a": 99, "b": 2}`
  is `{"a": 1, "b": 2}`, a fresh non-mutating map combining both sides
  with the left-hand map's values winning on overlapping keys — the
  map-typed counterpart to list `|` above, following the same
  "keys decide, left's values win" convention map `&`/`-` already use
  rather than `+`'s right-wins merge semantics; `|=` on a map target
  works for free through the same desugaring), and
  list symmetric difference via `^` (`[1, 2, 3] ^ [2, 3, 4]` is
  `[1, 4]`, a fresh non-mutating list deduping each side independently
  and returning left-only elements followed by right-only elements —
  set-style semantics mirroring the existing `symmetric_difference()`
  builtin, completing the list side of the `&`/`|`/`-`/`^` operator
  family; `^=` on a list target works for free through the same
  desugaring; `^` remains int-only bitwise-XOR when neither operand is
  a list), and map symmetric difference via the same `^` operator
  (`{"a": 1, "b": 2} ^ {"b": 3, "c": 4}` is `{"a": 1, "c": 4}`, a fresh
  non-mutating map keeping only keys present in exactly one side, each
...
- **Data structures**: lists `[1, 2, 3]` and maps `{"a": 1}`, `expr[expr]`
  ...
  internally (a bug fixed alongside `is_set`'s own addition); `to_set`
  converts a list into an actual `Set` runtime value, de-duplicating
  along the way — the last Set-completion gap now closed
- **Builtins**: `print`, `len`, `is_empty`, `type`, conversions, `push`, `pop`, `insert`,
  ...
  `nth_powerful_number` to return the powerful number found at a 1-indexed position via a bounded
  sequential scan with the every-prime-factor-exponent-at-least-2 check inlined from
  `is_powerful_number`'s own body, the value-returning sibling of `is_powerful_number`'s membership
  test,
  ...
  `nth_perfect_power` to return the perfect power found at a 1-indexed position via a bounded sequential scan reusing `is_perfect_power`'s own predicate, scoped to non-negative candidates only since `is_perfect_power` uniquely admits negative input and a single monotonic scan can't interleave both signs, the value-returning sibling of `is_perfect_power`'s membership test,
  ...
  `nth_smith_number` to return the Smith number found at a 1-indexed position, the
  value-returning sibling `is_smith_number` itself was missing, the same bounded sequential
  scan `nth_refactorable`/`nth_sphenic` already use,
  `is_carmichael_number` to test whether a composite, squarefree integer satisfies Korselt's
  ...
  `nth_vampire_number` to return the vampire number found at a 1-indexed position via a bounded
  ...
  `is_self_number` to test the Colombian/self-number property — no smaller integer `m` generates
  ...
  `itertools.combinations`, the enumerate-vs-count sibling of `binomial` for a specific size),
  `combinations_with_replacement` to return every r-length selection of a list that allows repeats
...
Actively developed, nightly. Recently landed: `zscore` (PR #449,
standardizing a numeric list to zero mean/unit variance — a
...
a list's minimum and maximum), and `diff` (PR #445, the inverse-shaped
sibling of `cumsum` — successive differences of a numeric list). See
[`CHANGELOG.md`](CHANGELOG.md) for the full merge history. Queued next
(see [`BACKLOG.md`](BACKLOG.md)), all breadth: `covariance` (task 1,
the two-list generalization of `variance`, sitting next to
`dot_product`), `correlation` (task 2, the normalized sibling of
`covariance`, rescaling it into `[-1, 1]` by the product of both
lists' standard deviations), `jaccard_similarity` (task 3, the
