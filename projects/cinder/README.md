...
Actively developed, nightly. Recently landed: a `|` (union) operator for
lists (PR #371, `[1, 2, 3] | [2, 3, 4]` is `[1, 2, 3, 4]`), the
set-style counterpart to list `&`/`-` above, mirroring the existing
`union()` builtin's dedupe-and-concatenate semantics — and before that
`is_luhn_valid` (PR #370, `is_luhn_valid("79927398713")` is `true`), a
Luhn mod-10 checksum validator for digit strings, the identifier-checksum
sibling of the digit-recurrence predicates above, operating on a string
of digits rather than a numeric value. See [`CHANGELOG.md`](CHANGELOG.md)
for the full merge history.
Coming up next (see [`BACKLOG.md`](BACKLOG.md)): `is_polydivisible`, a
...
digit-position math — a `^` (symmetric difference) operator for
lists (`[1, 2, 3] ^ [2, 3, 4]` is `[1, 4]`), completing the list side of
the set-style operator family alongside `&`/`|`/`-` above by mirroring
the existing `symmetric_difference()` builtin's dedupe-both-sides
semantics — `is_self_number`, the Colombian/self-number predicate
(`is_self_number(20)` is `true`, `is_self_number(21)` is `false`,
generated by `15 + digit_sum(15)`), the plain-digit-sum,
bounded-generator-search sibling of `is_happy_number`/`is_sad_number`'s
sum-of-squares cycle detection — a `|` (union) operator for maps
(`{"a": 1} | {"a": 99, "b": 2}` is `{"a": 1, "b": 2}`), the map-side
counterpart list `|` above leaves deferred, following the same
"left's values win" convention as map `&`/`-` rather than `+`'s
right-wins merge — `is_weird_number`, abundant but not semiperfect
(no subset of proper divisors sums to the number itself; `70` is the
smallest example), one step further into the perfect/abundant/deficient
family than `is_abundant`/`is_deficient` currently go — and a `^`
(symmetric difference) operator for maps (`{"a": 1, "b": 2} ^ {"b": 3,
"c": 4}` is `{"a": 1, "c": 4}`), completing the map side of the same
