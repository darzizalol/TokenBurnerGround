...
- **Operators**: full arithmetic/comparison/logical set, unary `+`
  ...
  `|=` on a list target works for free through the same desugaring;
  `|` remains int-only bitwise-OR when neither operand is a list)
...
Actively developed, nightly. Recently landed: `is_polydivisible` (PR
#372, `is_polydivisible(381654729)` is `true`), a digit-position
predicate checking that every decimal prefix of a number is divisible
by its own length, the prefix-checksum sibling of `is_disarium`'s own
digit-position math — and before that a `|` (union) operator for
lists (PR #371, `[1, 2, 3] | [2, 3, 4]` is `[1, 2, 3, 4]`), the
set-style counterpart to list `&`/`-` above, mirroring the existing
`union()` builtin's dedupe-and-concatenate semantics. See
[`CHANGELOG.md`](CHANGELOG.md) for the full merge history.
Coming up next (see [`BACKLOG.md`](BACKLOG.md)): a `^` (symmetric
difference) operator for lists (`[1, 2, 3] ^ [2, 3, 4]` is `[1, 4]`),
completing the list side of the set-style operator family alongside
`&`/`|`/`-` above by mirroring the existing `symmetric_difference()`
builtin's dedupe-both-sides semantics — `is_self_number`, the
...
`{"a": 1, "b": 2}`), the map-side counterpart list `|` above leaves
...
set-operator family once the two tasks above land, key-based since
there is no map-flavored `symmetric_difference()` builtin to mirror —
and `is_carmichael_number`, a Korselt's-criterion Fermat-pseudoprime
