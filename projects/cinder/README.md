...
Actively developed, nightly. Recently landed: a `^` (symmetric
...
`symmetric_difference()` builtin's dedupe-both-sides semantics — and
before that `is_polydivisible` (PR #372, `is_polydivisible(381654729)`
is `true`), a digit-position predicate checking that every decimal
prefix of a number is divisible by its own length, the prefix-checksum
sibling of `is_disarium`'s own digit-position math. See
[`CHANGELOG.md`](CHANGELOG.md) for the full merge history.
Coming up next (see [`BACKLOG.md`](BACKLOG.md)): `is_self_number`, the
Colombian/self-number predicate (`is_self_number(20)` is `true`,
`is_self_number(21)` is `false`, generated by `15 + digit_sum(15)`),
the plain-digit-sum, bounded-generator-search sibling of
`is_happy_number`/`is_sad_number`'s sum-of-squares cycle detection —
a `|` (union) operator for maps (`{"a": 1} | {"a": 99, "b": 2}` is
`{"a": 1, "b": 2}`), the map-side counterpart list `|` leaves
deferred, following the same "left's values win" convention as map
`&`/`-` rather than `+`'s right-wins merge — `is_weird_number`,
