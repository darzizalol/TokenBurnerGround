...
Actively developed, nightly. Recently landed: `is_luhn_valid` (PR #370,
`is_luhn_valid("79927398713")` is `true`), a Luhn mod-10 checksum
validator for digit strings, the identifier-checksum sibling of the
digit-recurrence predicates above, operating on a string of digits
rather than a numeric value — and before that a `&` (intersection)
operator for maps (PR #369, `{"a": 1, "b": 2} & {"a": 1, "c": 3}` is
`{"a": 1}`), the key-based counterpart to map `-` above, mirroring map
`-`'s own "keys decide, left's values win" convention. See
[`CHANGELOG.md`](CHANGELOG.md) for the full merge history.
Coming up next (see [`BACKLOG.md`](BACKLOG.md)): a `|` (union) operator
...
pandigital example) — a `^` (symmetric difference) operator for
...
`is_self_number`, the Colombian/self-number predicate
...
right-wins merge — and `is_weird_number`, abundant but not semiperfect
...
family than `is_abundant`/`is_deficient` currently go.
