...
Actively developed, nightly. Recently landed: a `|` (union) operator
for maps (PR #375, `{"a": 1} | {"a": 99, "b": 2}` is
`{"a": 1, "b": 2}`), the map-side counterpart list `|` left deferred,
following the same "left's values win" convention as map `&`/`-`
rather than `+`'s right-wins merge — and before that `is_self_number`
(PR #374, `is_self_number(20)` is `true`, `is_self_number(21)` is
`false`, generated by `15 + digit_sum(15)`), the plain-digit-sum,
bounded-generator-search sibling of `is_happy_number`/`is_sad_number`'s
sum-of-squares cycle detection — and before that a `^` (symmetric
difference) operator for lists (PR #373, `[1, 2, 3] ^ [2, 3, 4]` is
`[1, 4]`), completing the list side of the set-style operator family
alongside `&`/`|`/`-` above by mirroring the existing
`symmetric_difference()` builtin's dedupe-both-sides semantics. See
[`CHANGELOG.md`](CHANGELOG.md) for the full merge history.
Coming up next (see [`BACKLOG.md`](BACKLOG.md)): `is_weird_number`,
abundant but not semiperfect (no subset of proper divisors sums to
the number itself; `70` is the smallest example), one step further
into the perfect/abundant/deficient family than
`is_abundant`/`is_deficient` currently go — a `^` (symmetric
difference) operator for maps (`{"a": 1, "b": 2} ^ {"b": 3, "c": 4}`
is `{"a": 1, "c": 4}`), completing the map side of the same
...
predicate (`is_carmichael_number(561)` is `true`, the smallest
Carmichael number, `3 * 11 * 17` with `(p-1) | 560` for every prime
factor `p`), the composite-but-squarefree cousin of `is_prime` built
on the same trial-division factoring `prime_factors`/`is_smith_number`
already use — and a `<=>` (spaceship / three-way comparison) operator
(`1 <=> 2` is `-1`, `2 <=> 2` is `0`, `3 <=> 2` is `1`), built entirely
from the existing `<`/`==` comparison machinery so every type that's
already comparable (numbers, strings, lists, maps) gets it for free —
and `is_palindrome_permutation`, testing whether a string's characters
could be rearranged into some palindrome rather than already being one
...
`is_palindrome`'s single-string check.
