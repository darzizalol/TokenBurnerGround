...
Actively developed, nightly. Recently landed: `is_weird_number` (PR
#377, `is_weird_number(70)` is `true` — abundant but not semiperfect,
no subset of its own proper divisors sums back to it), one step
further into the perfect/abundant/deficient family than
`is_abundant`/`is_deficient`'s simple sum comparison, via a bounded
0/1 subset-sum sweep — and before that a `|` (union) operator for maps
(PR #375, `{"a": 1} | {"a": 99, "b": 2}` is `{"a": 1, "b": 2}`),
...
sum-of-squares cycle detection. See [`CHANGELOG.md`](CHANGELOG.md) for
the full merge history.
Coming up next (see [`BACKLOG.md`](BACKLOG.md)): a `^` (symmetric
...
already use — a `<=>` (spaceship / three-way comparison) operator
...
`is_palindrome`'s single-string check — and `is_practical_number`,
whether every integer from `1` to `n - 1` is a sum of distinct proper
divisors of `n` (`is_practical_number(6)` is `true`; `10` is not,
since no subset of `{1, 2, 5}` sums to `4`), the same subset-sum
reachability sweep `is_weird_number` itself uses.
